...
public abstract class BaseGetAllUseCase<TEntity, TResponse>(IBaseRepository<TEntity> repository) : IBaseUseCase<List<TResponse>>, IUseRepositoryUseCase<TEntity>
    where TEntity : AuditableBaseEntity
    where TResponse : class
{
    public IBaseRepository<TEntity> Repository => repository;

    public virtual async Task<List<TResponse>> ExecuteAsync()
    {
